  ...
  ReactDOM.render(
    <App
      ...
      onChangeTempoBy={(trackGroupName, delta) =>
        socket.emit("change_tempo_by", { trackGroupName, delta })
      }
      ...
      onSetLoopSpec={(loop_spec) => socket.emit("set_loop_spec", { loop_spec })}
      onSetMarkAtBar={(name, bar) =>
        socket.emit("set_mark_at_bar", { name, bar })
      }
      ...
      onRecChange={(track, enabled) =>
        socket.emit("set_rec", { track, enabled })
      }
      onRecSourceChange={(track, source) =>
        socket.emit("set_rec_source", { track, source })
      }
      onMuteChange={(track, enabled) =>
        socket.emit("set_mute", { track, enabled })
      }
      onSoloChange={(track, enabled) =>
        socket.emit("set_solo", { track, enabled })
      }
...
class UpperControlPanelRow extends Component {
  render() {
    ...
    return (
      <div className="upper-control-panel-row">
...
class MoreOptions extends Component {
  render() {
    const status_icon = this.props.audioIoRunning ? (
...
class LowerControlPanelRow extends Component {
  render() {
    return (
      <div className="lower-control-panel-row">
        <input
          className="image-button"
          type="image"
          src="/A.svg"
        ...
        {this.props.main_view_mode === "mixer" ? (
          ...
          <input
            className="image-button highlighted-button"
            type="image"
            src="/more.svg"
            onClick={(evt) => this.props.onToggleMainViewMode()}
          />
...
export class App extends Component {
  ...
  render() {
    ...
    const audible = calculate_tracks_audibility(
      tracks,
      this.props.is_transport_rolling
    );
    const meter_values = tracks.reduce((result, track) => {
      ...
      result[track.name] = {
        rms: rms_prefader + track.vol_dB,
        peak: peak_prefader + track.vol_dB,
      };
      return result;
    }, {});
    const playback_current_rms = tracks
      ...
      .map((factor) => factor_to_dB(factor));
  ...
  toggleMainViewMode() {
    this.setState({
      main_view_mode:
        this.state.main_view_mode === "mixer" ? "options" : "mixer",
    });
  }
}
