...
class SessionTracksPlayspecSource(PlayspecSource):
    def __init__(self, tracks: Iterable["manokee.track.Track"]):
        ...
        self._additional_data = {
            track: {"audible": track.is_solo if self._is_soloed else not track.is_mute}
    ...
    def create_entry(self, entry_number: int) -> PlayspecEntry:
        ...
        if clip is None:
            ...
            return PlayspecEntry(AudioClip.zeros(1, 1, 48000), 0, 1, 0, 0, 0.0, 0.0)
        if self._additional_data[track]["audible"]:
            fader = track.fader
        else:
            fader = Fader(float("-inf"))
        return PlayspecEntry(
            clip, 0, len(clip), 0, 0, fader.left_gain_factor, fader.right_gain_factor
        )
