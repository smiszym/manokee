...
class MidiInputReceiver(threading.Thread):
    ...
    def run(self):
        available_ports = [port for port in mido.get_input_names()
...
class VolumeChangeMessage(ManokeeMidiMessage):
    def __init__(self, track_number, new_value):
...
class ButtonMessage(ManokeeMidiMessage):
    def __init__(self, track_number, action):
        self.track_number = track_number
        self.action = action
...
class MidiInterpreter:
    ...
    def interpret(self, raw_message):
        if raw_message.type == 'control_change':
            if (self.first_track_vol_control
                    <= raw_message.control <= self.last_track_vol_control):
                return VolumeChangeMessage(
                    raw_message.control - self.first_track_vol_control,
                    raw_message.value * 30 / 127 - 20)
            elif (self.first_track_button_control
                    <= raw_message.control <= self.last_track_button_control
                    and raw_message.value == 0):
                return ButtonMessage(
                    raw_message.control - self.first_track_button_control,
                    self.bank)
